...
variable "num_workers" {
  description = "Number of worker nodes (null for autoscaling). For SINGLE_NODE clusters, this is automatically set to 0."
  type        = number
  default     = null
}
...
variable "cluster_mode" {
  description = <<-EOT
    Cluster mode: STANDARD or SINGLE_NODE.
    - STANDARD: Multi-node cluster with worker nodes (supports autoscaling)
    - SINGLE_NODE: Single-node cluster with no worker nodes (driver-only, runs Spark locally).
      For SINGLE_NODE clusters, num_workers is automatically set to 0 and autoscaling is disabled.
  EOT
  type        = string
  default     = "STANDARD"
