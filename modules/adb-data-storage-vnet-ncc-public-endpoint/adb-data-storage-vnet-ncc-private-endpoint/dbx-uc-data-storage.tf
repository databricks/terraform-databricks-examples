...
resource "azurerm_storage_account" "this" {
  ...
  # Ensure the storage account depends on the resource group being created first
...
resource "azurerm_storage_account_network_rules" "this" {
  ...
  # Default action for incoming requests (e.g., Deny)
  default_action = "Deny"

  # List of allowed IP addresses for the storage account
...
resource "azurerm_storage_container" "this" {
  ...
  # Ensure the container depends on the storage account being created first
...
resource "azurerm_user_assigned_identity" "this" {
  ...
  # Ensure the identity depends on the resource group being created first
  depends_on = [azurerm_resource_group.storage_rg]
}
...
resource "azurerm_role_assignment" "this" {
  ...
  # Ensure the role assignment depends on both the storage account and user-assigned identity being created first
...
resource "azurerm_databricks_access_connector" "this" {
  ...
  # Ensure the access connector depends on the user-assigned identity being created first
